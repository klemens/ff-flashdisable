...
MSBClass.changeState = function(newState) {
    if(australis) {
        // this does unfortunately not work in firefox 30 because of a regression
        // (it does work in ff 29, 31, 32, ....) -> Bug 983193
        //this.buttonImpl.state(this.buttonImpl, this.states[newState]);
        this.buttonImpl.label = this.states[newState].label;
        this.buttonImpl.icon = this.states[newState].icon;
    } else {
