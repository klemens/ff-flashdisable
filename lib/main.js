...
flashIcon = self.data.url("status_active.ico");
flashIconDisabled = self.data.url("status_disabled.ico");
flashIconNotFound = self.data.url("status_notfound.ico");
addonIcon = self.data.url("logo32.png");
            ...
            if("notfound" == status) {
                notifications.notify({
                    ...
                    onClick: function(data) {
                        require('sdk/deprecated/window-utils').activeBrowserWindow.BrowserOpenAddonsMgr("addons://detail/" + self.id);
                    }
                });
            }
        });
